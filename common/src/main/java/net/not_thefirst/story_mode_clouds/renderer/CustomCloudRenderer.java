    ...
    public void render(int skyColor, CloudStatus status, float cloudHeight, Matrix4f proj, Matrix4f modelView, Vec3 cam, float tickDelta) {
        ...
        for (int layer : order) {
            ...
            if (!type.doDepthWrite()) {
                drawLayer(ModRenderPipelines.POSITION_COLOR_NO_DEPTH, proj, modelView,
            ...
            drawLayer(
                ModRenderPipelines.CUSTOM_POSITION_COLOR, proj, modelView,
                    offX, (float) (layerConfiguration.LAYER_HEIGHT - cam.y), offZ,
                    currentLayer.buffer,
                    layer, currentLayer, skyColor);
            drawLayer(
    ...
    private void drawLayer(
        ...
        if (shader != null) {
            if (shader.getUniform("ModelOffset") != null) {
                shader.getUniform("ModelOffset").set(-ox, oy, -oz);
            }
            else {
