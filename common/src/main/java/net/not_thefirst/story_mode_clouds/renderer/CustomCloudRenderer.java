...
public class CustomCloudRenderer {
    ...
    protected static final ResourceLocation TEXTURE_LOCATION = 
        ResourceLocation.tryBuild("minecraft", "textures/environment/clouds.png");
    ...
    public void render(int skyColor, CloudStatus status, float cloudHeight, Matrix4f modelView, Vec3 cam, float tickDelta, PoseStack poseStack) {
        double baseDx = cam.x;
        ...
        for (int layer : order) {
            ...
            if (needs) {
                ...
                RenderedBuffer mesh = buildMeshForLayer(
            ...
            if (!type.doDepthWrite()) {
                drawLayer(ModRenderPipelines.POSITION_COLOR_NO_DEPTH, poseStack.last().pose(), modelView,
            ...
            drawLayer(
                ModRenderPipelines.CUSTOM_POSITION_COLOR, poseStack.last().pose(), modelView,
                    offX, (float) (layerConfiguration.LAYER_HEIGHT - cam.y), offZ,
                    currentLayer.buffer,
                    layer, currentLayer, skyColor);
            drawLayer(
    ...
    @Nullable
    private RenderedBuffer buildMeshForLayer(Texture.TextureData tex,
        ...
        RenderType pipe = ModRenderPipelines.CUSTOM_POSITION_COLOR;

        BufferBuilder bb = tess.getBuilder();
        bb.begin(pipe.mode(), pipe.format());
    ...
    private void drawLayer(
        RenderType rt, 
        Matrix4f proj, Matrix4f mv, 
        float ox, float oy, float oz, 
        VertexBuffer buf,
        int layer,
        LayerState currentLayer, int skyColor) {
        rt.setupRenderState();
        ShaderInstance shader = RenderSystem.getShader();
        
        if (shader != null) {
            if (shader.getUniform("ModelOffset") != null) {
                shader.getUniform("ModelOffset").set(0.0f, 0.0f, 0.0f);
            }
            else {
                System.out.println("MODEL_OFFSET uniform is null in CustomCloudRenderer.drawLayer");
            }

            if (shader.getUniform("Config") != null) {
                shader.getUniform("Config").set(packConfig(layer));
            }
            else {
                System.out.println("Config uniform is null in CustomCloudRenderer.drawLayer");
            }

            if (shader.getUniform("CloudFogStart") != null) {
                shader.getUniform("CloudFogStart").set(
                    (int) CloudsConfiguration.INSTANCE.getLayer(layer).FOG.FOG_START_DISTANCE
                );
            }
            else {
                System.out.println("CloudFogStart uniform is null in CustomCloudRenderer.drawLayer");
            }

            if (shader.getUniform("CloudFogEnd") != null) {
                shader.getUniform("CloudFogEnd").set(
                    (int) CloudsConfiguration.INSTANCE.getLayer(layer).FOG.FOG_END_DISTANCE
                );
            }
            else {
                System.out.println("CloudFogEnd uniform is null in CustomCloudRenderer.drawLayer");
            }

            if (shader.getUniform("CloudColor") != null) {
                shader.getUniform("CloudColor").set(
                    ARGB.redFloat(ColorUtils.getCloudShaderColor(layer, skyColor)), 
                    ARGB.greenFloat(ColorUtils.getCloudShaderColor(layer, skyColor)), 
                    ARGB.blueFloat(ColorUtils.getCloudShaderColor(layer, skyColor)), 
                    ARGB.alphaFloat(ColorUtils.getCloudShaderColor(layer, skyColor))
                );
            }
            else {
                System.out.println("CloudColor uniform is null in CustomCloudRenderer.drawLayer");
            }
        }
        else {
            System.out.println("Shader is null in CustomCloudRenderer.drawLayer");
            rt.clearRenderState();
            return;
        }
