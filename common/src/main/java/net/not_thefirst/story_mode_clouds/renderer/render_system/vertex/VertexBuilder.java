...
public class VertexBuilder {
    ...
    public static final boolean TRIANGLE_MODE = true;
    ...
    public static void quad(BuildingMesh bb, float x0, float y0, float z0,
        ...
        if (TRIANGLE_MODE) {
            triangle(bb, x0, y0, z0, x1, y1, z1, x2, y2, z2, r, g, b, a);
            triangle(bb, x0, y0, z0, x2, y2, z2, x3, y3, z3, r, g, b, a);
            return;
        }
    ...
    public static void quadColored(BuildingMesh bb, float x0, float y0, float z0,
        ...
        if (TRIANGLE_MODE) {
            triangleColored(bb, x0, y0, z0, x1, y1, z1, x2, y2, z2, c0, c1, c2);
            triangleColored(bb, x0, y0, z0, x2, y2, z2, x3, y3, z3, c0, c2, c3);
            return;
        }
    ...
    public static void quadNormal(BuildingMesh bb, float x0, float y0, float z0,
        ...
        if (TRIANGLE_MODE) {
            triangleNormal(bb, x0, y0, z0, x1, y1, z1, x2, y2, z2, c0, c1, c2);
            triangleNormal(bb, x0, y0, z0, x2, y2, z2, x3, y3, z3, c0, c2, c3);
            return;
        }
    ...
    public static void triangleNormal(BuildingMesh bb, float x0, float y0, float z0,
        ...
        float[] n = normals[0];
        int debugColor = 0xFF000000 | 
                     ((int)((n[0] * 0.5f + 0.5f) * 255) << 16) |
                     ((int)((n[1] * 0.5f + 0.5f) * 255) << 8)  |
                     ((int)((n[2] * 0.5f + 0.5f) * 255)); 

        for (int i = 0; i < 3; i++) {
            float[] pos = getVertexPosition(i, x0, y0, z0, x1, y1, z1, x2, y2, z2);
            int color = getVertexColor(i, c0, c1, c2);
            bb.addVertex(pos[0], pos[1], pos[2])
                .setNormal(normals[i][0], normals[i][1], normals[i][2])
                .setColor(color)
                ;
    ...
    private static int getVertexColor(int index, int c0, int c1, int c2) {
        int result = 0;
        switch (index) {
    ...
    private static float[][] computeNormals(
        ...
        float[] faceNormal = crossProduct(edges[0], edges[1]);
