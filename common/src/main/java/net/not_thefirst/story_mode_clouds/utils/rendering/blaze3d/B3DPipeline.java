...
public class B3DPipeline extends AbstractPipeline {
    ...
    public void setup() {
        ...
        for (var entry : uniformBlocks.entrySet()) {
            builder.withUniform(entry.getKey(), UniformType.UNIFORM_BUFFER);
        }
        ...
        builder.withColorWrite(maskState.writeColor())
            .withDepthWrite(maskState.writeDepth())
            .withCull(cullState == CullState.CULL)
            .withVertexFormat(B3DConversions.toVanillaVertexFormat(vertexFormat), B3DConversions.toPrimitiveTopology(primitive));
