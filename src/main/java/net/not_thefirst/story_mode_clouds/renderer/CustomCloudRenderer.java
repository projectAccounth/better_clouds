...
public class CustomCloudRenderer {
    ...
    */
    public void render(int cloudColor, CloudStatus status, float cloudHeight, Vec3 cam, float tickDelta, PoseStack poseStack) {
        ...
        for (int layer : order) {
            ...
            if (currentLayer.needsRebuild ||
                ...
                BufferBuilder mesh = buildMeshForLayer(tex, Tesselator.getInstance(), cellX, cellZ, status, layerPos, relY, layer, cloudColor, offX, offZ);
    ...
    @Nullable
    private BufferBuilder buildMeshForLayer(
